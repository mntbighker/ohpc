...
PartitionName=production QoS=production_qos AllowQoS=production_qos nodes="c[1-4]" Default=yes DefaultTime=01:00:00 MaxTime=1-0 State=UP DisableRootJobs=yes MaxMemPerCPU=3600 DefMemPerCPU=3500
PartitionName=debug QoS=debug_qos AllowQoS=debug_qos nodes="c[1-4]" Default=no DefaultTime=01:00:00 MaxTime=1-0 State=UP DisableRootJobs=yes MaxMemPerCPU=3600 DefMemPerCPU=3500
PartitionName=batch QoS=batch_qos AllowQoS=batch_qos nodes="c[1-4]" Default=no DefaultTime=01:00:00 MaxTime=3-0 State=UP DisableRootJobs=yes MaxMemPerCPU=3600 DefMemPerCPU=3500
PartitionName=long QoS=long_qos AllowQoS=long_qos nodes="c[1-4]" Default=no DefaultTime=01:00:00 MaxTime=5-0 State=UP DisableRootJobs=yes MaxMemPerCPU=3600 DefMemPerCPU=3500
...
wwsh file resync passwd shadow group slurm.conf
...
# -----------------------------------------
# Post Boot Node Conf (Section 6)
# -----------------------------------------
# Generate NHC configuration file based on compute node environment
# pdsh -w c[1-4] "/usr/sbin/nhc-genconf -H'*'-c -" | dshbak -c
